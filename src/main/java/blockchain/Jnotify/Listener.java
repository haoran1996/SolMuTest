...
public class Listener implements JNotifyListener{
    ...
    public static void gain_result(){
        try {
            //TestFileName
            TestFileName = "All files";
            TestFileContent = null;
            BranchCoverage = ReadSolcovInfo.GetTotalCoverage(ProjectPath);
            Coveredline = ReadSolcovInfo.getAllCoveredLines(ProjectPath);
            isMutantKilled = ReadSolcovInfo.GetTestCondition(ProjectPath);
            Thread.sleep(1000);
            System.out.println("运行完成！");
            System.out.println("正在将本次测试结果写入数据库。。。");
            MysqlUtil.InsertTestFile(MutatedFileName,MutationOperator,MutatedLine,MutatedFileContent,TestFileName,
                    TestFileContent,BranchCoverage,Coveredline,isMutantKilled,sql_statement);
        } catch (InterruptedException e) {
            e.printStackTrace();
